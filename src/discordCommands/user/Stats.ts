...
import { GuildMember, Role, Message, MessageEmbed, Collection } from 'discord.js';
...
export default class Stats extends Command {
	...
	public async action(message: Message): Promise<Message | Message[]> {
		if (message.member === null) return message.channel.send('Please login in order to check your Discord stats.');

		const guildMember: GuildMember = message.member;
		const status: string = guildMember.user.presence.status;
		const joinDiscord = `${moment(guildMember.user.createdAt).format('lll')}` +
		...
		const userRoles = new Collection(Array.from(message.member.roles.entries())
			.sort((a: [string, Role], b: [string, Role]) => b[1].position - a[1].position));

		const rolesNoBots = userRoles.filter((r: Role): boolean => r.name !== '@everyone' && r.managed === false);
		
		rolesNoBots.forEach((role: Role): void => { accurateRoles.push(role); });

		const embed: MessageEmbed = new MessageEmbed()
			.setColor(constants.embedColorBase)
			.setAuthor(`${guildMember.user.username}#${guildMember.user.discriminator}`)
			.setThumbnail(guildMember.user.avatarURL() || guildMember.user.defaultAvatarURL)
			.addField('Joined Server', joinServer, true)
			.addField('Joined Discord', joinDiscord, true)
			.addField('Roles', `${accurateRoles.join(', ')}`, false)
			.setFooter(`${guildMember.user.username} is currently ${Statuses[status as string]}.`);
